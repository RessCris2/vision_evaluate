<template>
    ...
    <div class='chooseimg'>
        ...
        <el-upload
            ...
            <template #trigger>
                <el-button size="small" type="primary">1、选择需要预测的图片和相应标注json文件</el-button>
            </template>
        </el-upload>
        ...
        <div>
            <el-button size="small" type="success" @click="submitUpload2">预测2</el-button>
        </div>
        <div>
    ...
    <!-- <div  class='chooselabel'>
      <el-upload
          ...
          :auto-upload="false"
          :on-exceed="handleLabelExceed"
          :on-change="handleLabelUpload2"
          >
        <template #trigger>
          <el-button size="small" type="primary">2、批量选择并上传标注 json 文件</el-button>
        </template>
      </el-upload>
    </div> -->
     ...
     <div class="eval">
      ...
      <!-- <el-button size="small" type="primary">2、选择并上传标注 json 文件</el-button> -->
        ...
        <!-- <el-button size="small" type="primary" @click="handleExport">4、导出结果</el-button> -->
        <!-- <div> -->
            <!-- <el-button size="small" type="primary" @click="handleExportTest">3、下载结果</el-button> -->
        <!-- </div> -->
    <!-- </div>  -->
    ...
    <!-- <div v-if="predictionLoaded && annotationsLoaded"> -->
        <div class="iou">
            <label> IoU </label>
            <el-input-number v-model="iou_thres" :min="0.5" :max="1" :step="0.05"  @change="handleConfOrIoUChange"/>
        </div>
    <div>
        <div class="batch_metrics">
        <h3>批量评估结果</h3>
        <el-table :data="batchMetricData" style="width: 100%">
    ...
    <div class="metrics">
        <h3>评估指标</h3>
        <!-- <el-input-number v-model="conf" :min="0.0" :max="1" :step="0.05" @change="handleConfOrIoUChange"/>-->
...
const fileUplodList = ref([]);
const imgFileList = computed(() => fileUplodList.value.filter((file) => file.name.endsWith('.png') || file.name.endsWith('.jpg'))
...
const fetchPredict2 =  (phrase_target, model_category, file) => {
...
const submitUpload2 = (file, fileList) => {
    ...
    fileList.forEach(file => {
        ...
        console.log('file', file);
        const response = fetchPredict2(value.value[0], value.value[1], file);
...
const fetchData2 = (conf, iou_thres, phrase_target, filename) => {
    const formData = new FormData();
    // formData.append('conf', conf);
    // formData.append('iou_thres', iou_thres);
    // formData.append('phrase_target', phrase_target);
...
const evalResultDict = reactive({});

const handleEvaluationBatch = () => {
    ...
    imgFileList.value.forEach((file) =>{
        console.log('conf', conf.value);
        console.log('iou_thres', iou_thres.value);
        console.log('value', value.value);
        console.log('file', file.name);
        const res = fetchData2(conf.value, iou_thres.value, value.value[0], file.name);
        res.then((response) => {
            evalResultDict[file.name] = convertResult(response.data);
            console.log('evalResultDict', evalResultDict);
            });
    });
    handleExportTest();
    }
...
const fetchUpload2 = async (labelme_file ) => {
    const formData = new FormData();
    ...
    // 通过 axios 或其他方式发送请求
    axiosTestInstance.post('/upload_file', formData, {
        headers: {
            'Content-Type': 'multipart/form-data',
        },
    })
...
const handleLabelUpload2 = (file, fileList) =>{
    // fetchUpload(predictionResult.value, file.raw); 
    fetchUpload2(file.raw);
...
//         return;
//     }
//     const res = fetchData(conf.value, iou_thres.value, value.value[0], fileName.value);
...
//         // console.log("fillColorArray.value", fillColorArray.value);
...
const fetchResult = async (filelist) => {
...
// const handleExport = () => {
//     console.log('imgFileList.value', imgFileList.value);
//     const filelist = imgFileList.value.map((file) => file.name);
...
const handleExportTest = async() => {
    ...
    console.log('response', response);
    batchMetricData.value = response;
